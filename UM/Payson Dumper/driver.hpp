...
namespace PaysonDRV {
	...
	bool Init() {
		DriverHandle = CreateFileW((L"\\\\.\\\{PaysonMemoryDumper}"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	bool ReadPhysical(PVOID address, PVOID buffer, DWORD size, DWORD* bytes_read = nullptr) {
		_rw arguments = { 0 };
		...
		DWORD returned = 0;
		BOOL ok = DeviceIoControl(DriverHandle, PaysonRead, &arguments, sizeof(arguments), &arguments, sizeof(arguments), &returned, NULL);

		if (bytes_read)
			*bytes_read = ok ? static_cast<DWORD>(arguments.size) : 0;
...
T read(uint64_t address) {
	T buffer{ };
	DWORD bytes_read = 0;
	if (PaysonDRV::ReadPhysical((PVOID)address, &buffer, sizeof(T), &bytes_read) && bytes_read == sizeof(T))
		return buffer;
	return T{};
}
...
{
	if (!PaysonDRV::DriverHandle || PaysonDRV::DriverHandle == INVALID_HANDLE_VALUE)
	{
		if (!PaysonDRV::Init())
		{
			return false;
